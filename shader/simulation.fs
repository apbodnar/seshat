...
void main(void) {
  ...
  float cut = pos_t0.a * distance(pos_t0.xy, center.xy) < 0.025 ? 0.0 : 1.0;
  // velocity verlet
  vec3 acc_t1 = acceleration(pos_t0.xyz);
  vec3 vel_t1 = vel_t0 + 0.5 * (acc_t0 + acc_t1) * DELTA_T;
  vec3 pos_t1 = floor(gl_FragCoord.xy) == vec2(0,dims-1.0) || floor(gl_FragCoord.xy) == vec2(dims-1.0,dims-1.0) ? pos_t0.xyz : pos_t0.xyz + vel_t1 * DELTA_T + 0.5 * acc_t1 * DELTA_T * DELTA_T;
